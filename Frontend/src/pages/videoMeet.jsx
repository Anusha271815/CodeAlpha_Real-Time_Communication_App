...
export default function VideoMeet(){
    ...
    let getUserMediaSuccess=(stream)=>{
        try{
            window.localstream.getTrack().forEach(track => track.stop())
        ...
        stream.getTracks().forEach(track=>track.onended=()=>{
            ...
            for(let id in connections){
                ...
                connections[id].createOffer().then((description)=>{
                    connections[id].setLocalDescription(description)
                    .then(()=>{
                        socketRef.current.emit("current",id,JSON.stringify({"sdp":connections[id].localDescription}))
                    }).catch(e=>console.log(e))
                })
            }
    ...
    let gotMessageFromServer=()=>{
        ...
        if(fromId !== socketIdRef.current){
            ...
            if(signal.ice){
                connections[fromId].addIceCandidate(newRTCIceCandidate(signal.ice)).catch(e=>console.log(e));
            }
        }
    }
    ...
    let connectToSocketServer=()=>{
        ...
        socketRef.current.on("connect", ()=>{
            ...
            socketRef.current.on("user-defined",(id,clients)=>{
                clients.forEach((socketListId)=>{
                    connections[socketListId]=new RTCPeerConnection(peerConficConnections)
                    ...
                    if(window.localStream !==undefined && window.localStream !==null){
                        connections[socketListId].addStream(windows.localStream);
                    }else{
                        //blacksilence
        ...
        <div>
            ...
            <div>
                ...
                <video
                    ...
                    autoPlay
                    muted
                    playsInline   // important for Chrome and mobile
